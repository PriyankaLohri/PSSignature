...
@IBDesignable
final public class PSDrawSignatureView: UIView {
    ...
    public func changeColor(textColor:UIColor){
    ...
    func addPlaceHolder(placeHolderString:String, frame:CGRect)  {
        if(doesContainSignature == false){
            lblPlaceHolder.text = placeHolderString.uppercased()
            lblPlaceHolder.textColor = UIColor(red: 202.0/255.0, green: 202.0/255.0, blue: 202.0/255.0, alpha: 1.0)
            lblPlaceHolder.textAlignment = .center
            lblPlaceHolder.frame = frame
            lblPlaceHolder.numberOfLines = 3
            lblPlaceHolder.lineBreakMode = .byWordWrapping
//            lblPlaceHolder.font
//            lblPlaceHolder.updateFontWithTraits(label:lblPlaceHolder, actualFont: UIFont(name:"Knockout-HTF33-JuniorHeviwt", size:18.0)!)

            if(!lblPlaceHolder.isDescendant(of: self)) {
